...
class DCGAN:
    def __init__(self):
        ...
        self.cross_entropy = tf.losses.BinaryCrossentropy()
    ...
    def build_discriminator(self): # 識別器

        model = tf.keras.models.Sequential([

            tf.keras.layers.Conv2D(32,kernel_size=3 ,strides=2, input_shape=self.img_shape, padding="same"), # 28*28*1を14*14*32のテンソルにするたたみ込み層
            tf.keras.layers.LeakyReLU(alpha=0.01),# LeakyReLUによる活性化

            tf.keras.layers.Conv2D(64, kernel_size=3, strides=2, input_shape=self.img_shape, padding="same"),# 14*14*32を7*7*64のテンソルにするたたみ込み層
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(alpha=0.01),

            tf.keras.layers.Conv2D(128, kernel_size=3, strides=2, input_shape=self.img_shape, padding="same"),# 7*7*64を3*3*128のテンソルにするたたみ込み層
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.LeakyReLU(alpha=0.01),
    ...
    def discriminator_loss(self, real_output, fake_output): # 識別器誤差
        real_loss = self.cross_entropy(tf.ones_like(real_output), real_output)
        fake_loss = self.cross_entropy(tf.zeros_like(fake_output), fake_output)
        total_loss = real_loss + fake_loss
        return total_loss

    def generator_loss(self, fake_output): # 生成器誤差
    ...
    def train(self, iterations, batch_size, sample_interval):
        log_dir = "./logs/dcgan/" + datetime.datetime.now(pytz.timezone('Asia/Tokyo')).strftime("%Y%m%d-%H%M%S")
        ...
        #with self.summary_writer.as_default():
        #    tf.summary.trace_export(name="dcgan",step=0,profiler_outdir=log_dir)
...
if __name__ == "__main__":
    ...
    dcgan = DCGAN()
    dcgan.train(iterations, batch_size, sample_interval)