...
class SGAN():
    def __init__(self):
        ...
        self.dataset = sgan.Dataset(self.num_labeled)
    ...
    def train(self, iterations,batch_size,sample_interval):
        ...
        for iteration in range(iterations):
            ...
            if (iteration + 1) % sample_interval == 0:
                ...
                if (iteration + 1) == sample_interval:
                    disc_gif = show_hidden_outputs.marge_hidden_outputs_gif(self.discriminator_supervised.layers[0],pred_img)
                    gen_gif = show_hidden_outputs.marge_hidden_outputs_generator_gif(self.generator,pred_z)
                    for i in range(len(disc_gif)):
                        disc_gif[i] = [disc_gif[i]]
                    for i in range(len(gen_gif)):
                        gen_gif[i] = [gen_gif[i]]

                else:
                    add_disc_gif = show_hidden_outputs.marge_hidden_outputs_gif(self.discriminator_supervised.layers[0],pred_img)
                    add_gen_gif = show_hidden_outputs.marge_hidden_outputs_generator_gif(self.generator,pred_z)
                    for i in range(len(disc_gif)):
                        disc_gif[i].append(add_disc_gif[i])
                    for i in range(len(gen_gif)):
                        gen_gif[i].append(add_gen_gif[i])
        ...
        for i, l in enumerate(self.discriminator_supervised.layers[0].layers):
            block = amp.blocks.Imshow(disc_gif[i])
            anim = amp.Animation([block])

            anim.controls()
            anim.save_gif(l.name)
            plt.show()
        for i, l in enumerate(self.generator.layers):
            block = amp.blocks.Imshow(gen_gif[i])
            anim = amp.Animation([block])
...
if __name__ == "__main__":
    ...
    """
    model = sgan.discriminator_supervised
    optimizer = tf.keras.optimizers.Adam()

    checkpoint_path = './checkpoint/sgan'
    ...
    ckpt_manager = tf.train.CheckpointManager(root, checkpoint_path, max_to_keep=1)
    ckpt_manager.save()
